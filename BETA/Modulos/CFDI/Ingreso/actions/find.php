<?php
	...
	if($_SESSION){
		switch ($_REQUEST['AC']) {
			case "Fcli":
				$cli = $drv->resultadoQuery("SELECT id, rfc, razons FROM tbl_clients WHERE idEmisor=".$_SESSION['usrID']." AND (rfc like '%".addslashes($_REQUEST['term'])."%' OR razons like '%".addslashes($_REQUEST['term'])."%') AND pass<>1 ORDER BY rfc ASC LIMIT 10", BD_mysql::BD_TABLA);
				if($cli){
					foreach($cli AS $c)
			...
			break;
			case "FindI":
				$data = $drv->resultadoQuery("Select E.id, E.serie, E.folio, E.total, E.moneda, E.estado, E.fecha, C.rfc, C.razons, T.status, T.uuid, T.error, R.error as Derr from fac_tbl_emitidas  as E inner join tbl_clients as C on E.idCliente = C.id and E.idEmisor = C.idemisor LEFT JOIN fac_adm_control as T on T.idEmisor = E.idEmisor and T.idFactura = E.id Left Join fac_adm_errores as R on R.codigo = T.error where E.serie in (SELECT serie FROM fac_tbl_series WHERE tipo='I') and E.idCliente = '".$_POST['n']."' and E.idEmisor = '".$_SESSION['usrID']."' Order by E.id desc Limit 10" ,BD_mysql::BD_TABLA);
				if($data){
					$re['res'] = '<div class="table-responsive"><table class="table table-sm table-striped table-hover"><thead class="thead-dark text-center text-white"><tr><th width="5%" style="border-radius: 7px 0 0 0;" scope="col"></th><th width="20%" scope="col">Folio</th><th width="25%" scope="col">Receptor</th><th width="15%" scope="col">Fecha</th><th width="35%" style="border-radius: 0 7px 0 0;" scope="col">Opciones</th></tr></thead><tbody>';
					foreach($data as $dat){
						switch ($dat->estado){
							case 0:
							break;
							...
							break;
							case 4:
								$options = '<a class="text-success" data-toggle="tooltip" data-placement="bottom" data-original-title="Enviar" onclick="envio(\''.$dat->id.'\', \''.$dat->rfc.'\', \''.$dat->serie.'-'.$dat->folio.'\', \'$ '.number_format($dat->total,$_SESSION['deciX'],'.',',').' '.$dat->moneda.'\')"><i class="far fa-envelope"></i></a><a class="text-2danger" data-toggle="tooltip" data-placement="bottom" href="/CFDI/Ingreso/PDF/'.$dat->id.'" target="_blank" data-original-title="Vista previa PDF"><i class="far fa-file-pdf"></i></a><a class="text-warning" data-toggle="tooltip" data-placement="bottom" onclick="downxml(\''.$dat->id.'\')" href = "" data-original-title="Vista previa XML"><i class="far fa-file-archive"></i></a><a class="text-danger" data-toggle="tooltip" data-placement="bottom" onclick="delDoc('.$dat->id.',\''.$dat->rfc.'\',\''.$dat->serie.'-'.$dat->folio.'\',\''.$dat->total.'\');" data-original-title="Eliminar del sistema"><i class="far fa-trash-alt fa-2x" aria-hidden="true"></i></a>';
								$trcolor = '<small class="font-weight-bold text-danger">Error:</small><span data-toggle="tooltip" data-placement="bottom" title="" data-original-title="'.$dat->Derr.'"> '.$dat->error.'</span>';
							break;
						}
						$re['res'] .= '<tr class="text-black-50 text-center"><td scope="row"><div class="checkbox checkbox-primary checkbox-circle"><input id="checkbox1" type="checkbox" name="'.$dat->id.'" value="'.$dat->id.'" '.($dat->estado == 0 ? "disabled" : "").'"></div></td><th scope="row">'.$dat->serie.' - '.$dat->folio.'<br> $'.$dat->total.' '.($dat->moneda == "XXX" ? "" : "<b class='text-success'>".$dat->moneda).'</b><br>'.$trcolor.'</th><td scope="row"> '.$dat->rfc.'<br>'.$dat->razons.'</td><td scope="row"> '.$dat->fecha.'</td><td scope="row" class="btnPlg">'.$options.'</td></tr>';
					}
					$re['res'] .= "</tbody></table></div>";
					$re['estado'] = 1;
				} else {
					$re['res'] = '<div class="table-responsive"><table class="table table-sm table-striped table-hover"><thead class="thead-dark text-center text-white"><tr><th width="5%" style="border-radius: 7px 0 0 0;" scope="col"></th><th width="20%" scope="col">Folio</th><th width="25%" scope="col">Receptor</th><th width="15%" scope="col">Fecha</th><th width="35%" style="border-radius: 0 7px 0 0;" scope="col">Opciones</th></tr></thead><tbody><tr class="text-black-50 text-center"><td colspan="5">"Sin coincidencias"<td></tr></tbody></table></div>';
					$re['estado'] = 1;
				}
			break;
			/*
